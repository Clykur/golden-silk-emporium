    ...
    <DashboardLayout title="Account Overview" subtitle="Dashboard">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="border border-border bg-champagne/10 p-5 hover:border-gold/30 transition-colors"
          >
            ...
            {recentOrders.map((order: any) => (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <span
                    className={`border px-2 py-0.5 text-[10px] uppercase tracking-wider rounded ${ORDER_STATUS_COLORS[order.status] || "bg-muted text-muted-foreground border-border"}`}
                  >
                    {order.status}
                  </span>
