...
import type {
  FilterState, ProductFormData, OrderItem, ShippingAddress, OrderStatus,
  CustomerAddress, SupportTicket, SupportMessage, Notification, AuditLog, ReturnRequest,
} from "./types";
...
export const productsApi = {
  ...
  async getBestsellers(limit = 8) {
    return productsApi.list({ isBestseller: true, sort: "featured" }).then((p) => p.slice(0, limit));
  },
  ...
  async create(form: ProductFormData) {
    ...
    if (images.length > 0) {
      const { error: imgError } = await supabase
        .from("product_images")
        .insert(images.map((img, i) => ({ url: img.url, alt_text: img.alt_text, is_featured: img.is_featured, product_id: product.id, sort_order: i })));
    ...
    if (images !== undefined) {
      ...
      if (images.length > 0) {
        await supabase.from("product_images").insert(
          images.map((img, i) => ({ url: img.url, alt_text: img.alt_text, is_featured: img.is_featured, product_id: id, sort_order: i }))
        );
      }
    }
  },
  ...
  async duplicate(id: string) {
    ...
    return productsApi.create({ ...rest, slug: newSlug, sku: newSku, status: "draft", images: images || [] });
  },
...
export const categoriesApi = {
  ...
  async update(id: string, data: Partial<{ name: string; slug: string; description: string; image: string; sort_order: number; is_active: boolean }>) {
...
export const collectionsApi = {
  ...
  async create(data: { name: string; slug: string; tagline?: string; description?: string; image?: string; is_featured?: boolean; sort_order?: number }) {
    const { data: col, error } = await supabase.from("collections").insert(data).select().single();
    if (error) throw error;
    return col;
  },

  async update(id: string, data: Partial<{ name: string; slug: string; tagline: string; description: string; image: string; is_featured: boolean; sort_order: number; is_active: boolean }>) {
...
export const ordersApi = {
  ...
  async updatePayment(id: string, payment: { razorpay_payment_id: string; razorpay_signature: string; payment_status: string }) {
    const { error } = await supabase.from("orders").update({ ...payment, status: "processing" }).eq("id", id);
    if (error) throw error;
  },
...
export const returnsApi = {
  ...
  async updateStatus(id: string, status: string, refund_amount?: number, admin_notes?: string) {
    const { error } = await supabase
      .from("return_requests")
      .update({ status, ...(refund_amount ? { refund_amount } : {}), ...(admin_notes ? { admin_notes } : {}) })
      .eq("id", id);
    if (error) throw error;
  },
};
...
export const reviewsApi = {
  ...
  async submit(review: { product_id: string; rating: number; title?: string; comment?: string; reviewer_name?: string; reviewer_email?: string; user_id?: string }) {
...
export const couponsApi = {
  async validate(code: string, cartTotal: number) {
    ...
    if (cartTotal < data.min_order_value) throw new Error(`Minimum order of ₹${data.min_order_value} required`);
    if (data.usage_limit && data.usage_count >= data.usage_limit) throw new Error("Coupon usage limit reached");
  ...
  async create(coupon: Omit<import("./types").Coupon, "id" | "created_at" | "usage_count">) {
    const { data, error } = await supabase.from("coupons").insert({ ...coupon, code: coupon.code.toUpperCase() }).select().single();
    if (error) throw error;
    return data;
  },
...
export const cartApi = {
  ...
  async upsert(userId: string, productId: string, quantity: number, size: string) {
    const { error } = await supabase.from("cart_items").upsert(
      { user_id: userId, product_id: productId, quantity, size, updated_at: new Date().toISOString() },
  ...
  async remove(userId: string, productId: string, size: string) {
    const { error } = await supabase.from("cart_items")
      .delete().eq("user_id", userId).eq("product_id", productId).eq("size", size);
    if (error) throw error;
  },
...
export const adminStatsApi = {
  async getOverview() {
    ...
    return { totalRevenue, totalOrders, publishedProducts, lowStockProducts, outOfStock, totalCustomers, monthlyData };
  },
};
