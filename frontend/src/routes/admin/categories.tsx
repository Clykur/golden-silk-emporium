...
function AdminCategories() {
  ...
  const { data: cats = [], isLoading } = useQuery({ queryKey: ["admin-categories"], queryFn: categoriesApi.adminList });

  const createMut = useMutation({
    mutationFn: (d: typeof empty) => categoriesApi.create(d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-categories"] }); toast.success("Category created"); resetForm(); },
    onError: (e: any) => toast.error(e.message),
  });

  const updateMut = useMutation({
    mutationFn: ({ id, d }: { id: string; d: Partial<typeof empty> }) => categoriesApi.update(id, d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-categories"] }); toast.success("Category updated"); resetForm(); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const startEdit = (c: Category) => { setForm({ name: c.name, slug: c.slug, description: c.description || "", image: c.image || "" }); setEditId(c.id); setShowForm(true); };
      ...
      {showForm ? (
          ...
          <div className="border border-border p-6 space-y-4 bg-background">
            <label className="block">
              ...
              <input value={form.name} onChange={(e) => { setField("name", e.target.value); if (!editId) setField("slug", slugify(e.target.value)); }}
                required className={input} placeholder="Kanjivaram Sarees" />
            </label>
            <label className="block">
              <span className="eyebrow text-[10px] mb-1.5 block">Slug</span>
              <input value={form.slug} onChange={(e) => setField("slug", e.target.value)} className={input} placeholder="kanjivaram-sarees" />
            </label>
            <label className="block">
            ...
            <label className="block">
              ...
              {form.image && <img src={form.image} className="h-32 w-full object-cover border border-border mb-2" />}
              <div className="flex gap-2">
                <input value={form.image} onChange={(e) => setField("image", e.target.value)} className={input + " flex-1"} placeholder="https://... or upload" />
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {isLoading ? (
            ...
            <div key={c.id} className="border border-border bg-background hover:shadow-[var(--shadow-card)] transition-shadow">
              {c.image && <img src={c.image} className="h-32 w-full object-cover" />}
              {!c.image && <div className="h-32 bg-champagne/20 flex items-center justify-center"><Tag className="h-8 w-8 text-muted-foreground/40" /></div>}
              <div className="p-4">
                ...
                <div className="flex gap-2 mt-4">
                  <button onClick={() => startEdit(c)} className="flex-1 border border-border py-2 text-xs hover:bg-muted transition-colors flex items-center justify-center gap-1">
