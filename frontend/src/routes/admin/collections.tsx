...
const empty = { name: "", slug: "", tagline: "", description: "", image: "", is_featured: false, sort_order: 0 };
...
function AdminCollections() {
  ...
  const { data: collections = [], isLoading } = useQuery({ queryKey: ["admin-collections"], queryFn: collectionsApi.adminList });

  const createMut = useMutation({
    mutationFn: (d: typeof empty) => collectionsApi.create(d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-collections"] }); toast.success("Collection created"); resetForm(); },
    onError: (e: any) => toast.error(e.message),
  });

  const updateMut = useMutation({
    mutationFn: ({ id, d }: { id: string; d: Partial<typeof empty> }) => collectionsApi.update(id, d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-collections"] }); toast.success("Collection updated"); resetForm(); },
    onError: (e: any) => toast.error(e.message),
  });

  const deleteMut = useMutation({
    mutationFn: collectionsApi.delete,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-collections"] }); toast.success("Collection deleted"); },
    onError: (e: any) => toast.error(e.message),
  });

  const resetForm = () => { setForm(empty); setEditId(null); setShowForm(false); };
  const startEdit = (c: Collection) => {
    setForm({ name: c.name, slug: c.slug, tagline: c.tagline || "", description: c.description || "", image: c.image || "", is_featured: c.is_featured, sort_order: c.sort_order });
  ...
  return (
    <AdminLayout
      title={showForm ? (editId ? "Edit Collection" : "New Collection") : "Collections"}
      subtitle={showForm ? "" : `${collections.length} collections`}
      actions={
        showForm ? (
      ...
      {showForm ? (
        <form onSubmit={handleSubmit} className="max-w-lg space-y-4">
          <div className="border border-border p-6 space-y-4 bg-background">
            <label className="block">
              ...
              <input value={form.name} onChange={(e) => { setField("name", e.target.value); if (!editId) setField("slug", slugify(e.target.value)); }}
                required className={input} placeholder="Heritage Weaves" />
            </label>
            <label className="block">
              <span className="eyebrow text-[10px] mb-1.5 block">Slug</span>
              <input value={form.slug} onChange={(e) => setField("slug", e.target.value)} className={input} />
            </label>
            <label className="block">
              <span className="eyebrow text-[10px] mb-1.5 block">Tagline</span>
              <input value={form.tagline} onChange={(e) => setField("tagline", e.target.value)} className={input} placeholder="Handloom masterworks from Banaras & Kanchipuram" />
            </label>
            <label className="block">
              <span className="eyebrow text-[10px] mb-1.5 block">Description</span>
              <textarea value={form.description} onChange={(e) => setField("description", e.target.value)} rows={3} className={input + " resize-y"} />
            </label>
            <div>
              ...
              <div className="flex gap-2">
                <input value={form.image} onChange={(e) => setField("image", e.target.value)} className={input + " flex-1"} placeholder="https://..." />
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <div onClick={() => setField("is_featured", !form.is_featured)} className={`relative h-5 w-9 rounded-full transition-colors ${form.is_featured ? "bg-gold" : "bg-border"}`}>
            ...
            <label className="block">
              ...
              <input type="number" value={form.sort_order} onChange={(e) => setField("sort_order", parseInt(e.target.value) || 0)} className={input} min={0} />
            </label>
          </div>
        </form>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2">
          {isLoading ? (
            ...
            <div key={c.id} className="border border-border bg-background relative hover:shadow-[var(--shadow-card)] transition-shadow">
              <div className="aspect-video relative overflow-hidden">
                {c.image ? (
                  <img src={c.image} className="h-full w-full object-cover" />
                ) : (
                  <div className="h-full bg-champagne/20 flex items-center justify-center"><Layers className="h-10 w-10 text-muted-foreground/30" /></div>
                )}
                {c.is_featured && (
              ...
              <div className="p-4">
                ...
                {c.tagline && <p className="text-xs text-muted-foreground mt-1 italic">{c.tagline}</p>}
                <p className="text-[10px] text-muted-foreground mt-1">/{c.slug} · Order: {c.sort_order}</p>
                <div className="flex gap-2 mt-4">
                  <button onClick={() => startEdit(c)} className="flex-1 border border-border py-2 text-xs hover:bg-muted flex items-center justify-center gap-1">
...
const input = "w-full border border-border bg-background px-3 py-2.5 text-sm focus:outline-none focus:border-foreground transition-colors";
