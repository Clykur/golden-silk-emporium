...
export const Route = createFileRoute("/admin/")({
  head: () => ({
    meta: [{ title: "Admin Dashboard — Drapeva" }],
  }),
...
function AdminDashboard() {
  ...
  const revenueChange = prevRevenue > 0 ? (((recentRevenue - prevRevenue) / prevRevenue) * 100).toFixed(1) : "0";
    ...
    <AdminLayout title="Dashboard" subtitle="Real-time store performance">
      <div className="space-y-8">
        ...
        {(stats.lowStockProducts > 0 || stats.outOfStock > 0) && (
          <div className="flex items-center gap-3 border border-amber-200 bg-amber-50 px-5 py-4 rounded text-sm">
            ...
            <span className="text-amber-800">
              ...
              {stats.lowStockProducts > 0 && <>{stats.lowStockProducts} products with low stock (≤5 units).</>}
              {" "}
              <a href="/admin/inventory" className="underline font-medium">Manage inventory →</a>
            </span>
          </div>
        )}
          ...
          <div className="lg:col-span-2 border border-border bg-background p-6">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={stats.monthlyData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                  <defs>
                  ...
                  <YAxis stroke="#8c7853" fontSize={10} tickLine={false} tickFormatter={(v) => `₹${(v / 100000).toFixed(0)}L`} />
                  <Tooltip formatter={(v: any) => [formatINR(v), "Revenue"]} labelStyle={{ fontSize: 11 }} />
                  <Area type="monotone" dataKey="sales" stroke="#d4af37" strokeWidth={2} fill="url(#revenueGrad)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "Add New Product", href: "/admin/products", desc: "Publish to store instantly", icon: ShoppingBag },
            { label: "Manage Orders", href: "/admin/orders", desc: "Update statuses & tracking", icon: Package },
            { label: "Review Moderation", href: "/admin/reviews", desc: "Approve customer reviews", icon: Users },
            { label: "Create Coupon", href: "/admin/coupons", desc: "Launch discount campaigns", icon: DollarSign },
          ].map((item) => (
            <a
              key={item.href}
              href={item.href}
