...
function AdminInventory() {
  ...
  const updateStockMut = useMutation({
    mutationFn: ({ id, quantity }: { id: string; quantity: number }) => productsApi.updateStock(id, quantity),
  ...
  const lowStock = products.filter((p: Product) => p.stock_quantity > 0 && p.stock_quantity <= 5).length;
        ...
        {(outOfStock > 0 || lowStock > 0) && (
          <div className="flex items-start gap-3 border border-amber-200 bg-amber-50 px-5 py-4 rounded text-sm">
            ...
            <div className="text-amber-800">
              {outOfStock > 0 && <p><strong>{outOfStock} products are out of stock.</strong> Restock immediately to avoid lost sales.</p>}
            ...
            <table className="w-full text-sm text-left border-collapse">
              ...
              <tbody className="divide-y divide-border">
                {filtered.map((p: Product) => {
                  ...
                  const stockStatus = p.stock_quantity === 0 ? "out" : p.stock_quantity <= 5 ? "low" : "ok";
                  return (
                    <tr key={p.id} className={`hover:bg-champagne/5 ${stockStatus === "out" ? "bg-red-50/30" : stockStatus === "low" ? "bg-amber-50/30" : ""}`}>
                      <td className="p-4">
                        <img src={p.image || "/placeholder-saree.jpg"} className="h-10 w-8 object-cover border border-border" />
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4">
                        <span className={`text-base font-display ${stockStatus === "out" ? "text-destructive" : stockStatus === "low" ? "text-amber-600" : ""}`}>
                          {p.stock_quantity}
                        </span>
                      ...
                      <td className="p-4">
                        <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded border ${
                          stockStatus === "out" ? "bg-red-50 text-red-600 border-red-200" :
                          stockStatus === "low" ? "bg-amber-50 text-amber-600 border-amber-200" :
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <div className="flex items-center border border-border">
                            <button onClick={() => setEditing((e) => ({ ...e, [p.id]: String(Math.max(0, parseInt(e[p.id] || String(p.stock_quantity)) - 1)) }))}
                              className="px-2 py-1.5 hover:bg-muted transition-colors"><ArrowDown className="h-3 w-3" /></button>
                            <input
                              type="number"
                              value={isEditing ? editing[p.id] : p.stock_quantity}
                              onChange={(e) => setEditing((ev) => ({ ...ev, [p.id]: e.target.value }))}
                              onFocus={() => { if (!isEditing) setEditing((e) => ({ ...e, [p.id]: String(p.stock_quantity) })); }}
                              className="w-16 bg-transparent px-2 py-1.5 text-sm text-center focus:outline-none"
                              min={0}
                            />
                            <button onClick={() => setEditing((e) => ({ ...e, [p.id]: String(parseInt(e[p.id] || String(p.stock_quantity)) + 1) }))}
                              className="px-2 py-1.5 hover:bg-muted transition-colors"><ArrowUp className="h-3 w-3" /></button>
                          </div>
                          {isEditing && (
                            <button onClick={() => handleStockUpdate(p.id)}
