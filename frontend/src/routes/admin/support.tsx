...
import { HelpCircle, MessageSquare, Send, ChevronDown, ChevronUp, Clock, CheckCircle2, AlertCircle } from "lucide-react";
...
export const Route = createFileRoute("/admin/support")({
  head: () => ({
    meta: [{ title: "Support Tickets — Drapeva Admin" }],
  }),
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {filtered.map((ticket: SupportTicket) => {
            ...
            const statusColor = STATUS_COLORS[ticket.status] || "bg-muted text-muted-foreground border-border";
            return (
                ...
                >
                  <div>
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <span className={`border px-2 py-0.5 text-[10px] uppercase tracking-wider rounded ${statusColor}`}>
                ...
                {expanded && (
                  <div className="border-t border-border bg-champagne/5 p-5 space-y-4">
                      ...
                      {(ticket.messages || []).map((msg) => (
                        <div
                          ...
                          className={`p-3 rounded text-sm ${
                            msg.sender_type === "customer" ? "bg-background border border-border mr-12" : "bg-gold/10 border border-gold/20 ml-12"
                          }`}
                        >
                          <p className="text-xs font-medium mb-1">
                            {msg.sender_type === "customer" ? `Customer — ${msg.sender_name}` : `Drapeva Support — ${msg.sender_name}`}
                            {" · "}
                    ...
                    <div className="border-t border-border pt-4">
                      <p className="text-xs uppercase tracking-wider text-muted-foreground mb-2">Reply to Customer</p>
