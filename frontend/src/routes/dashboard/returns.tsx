...
export const Route = createFileRoute("/dashboard/returns")({
  head: () => ({
    meta: [
      { title: "Returns & Refunds — Drapeva" },
      { name: "description", content: "Request returns and track refunds." },
    ],
  }),
  component: Returns,
});
...
function Returns() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowForm(false);
      setSelectedOrderId(""); setReason(RETURN_REASONS[0]); setComments("");
    },
    onError: (e: any) => toast.error(e.message || "Failed to submit return request"),
  });

  return (
    <DashboardLayout title="Returns & Refunds" subtitle="My Returns">
      {/* Policy Note */}
      <div className="flex gap-3 border border-amber-200 bg-amber-50 p-4">
        ...
        <div className="text-sm text-amber-800">
          ...
          <p className="text-xs leading-relaxed">
            Returns are accepted within 7 days of delivery for unused, unwashed items in original packaging.
            Sarees that have been draped or altered are not eligible for return. Custom/made-to-order pieces are final sale.
          </p>
        </div>
      </div>
      ...
      {showForm && (
        ...
        >
          <div className="flex justify-between items-center border-b border-border pb-4">
            ...
            <button type="button" onClick={() => setShowForm(false)} className="text-muted-foreground hover:text-foreground text-xs uppercase tracking-wider">
              Cancel
            </button>
          </div>
            ...
            >
              ...
              {deliveredOrders.map((o: any) => (
                <option key={o.id} value={o.id}>
                  #{o.id.slice(0, 8).toUpperCase()} — {new Date(o.created_at).toLocaleDateString("en-IN")} — {formatINR(o.total)}
                </option>
              ))}
            </select>
          </label>
          ...
          <label className="block">
            <span className="eyebrow mb-1 block">Additional Comments <span className="normal-case text-muted-foreground">(optional)</span></span>
            <textarea
              rows={3}
              value={comments}
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button type="submit" disabled={createMut.isPending || !selectedOrderId}
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2].map((i) => <div key={i} className="border border-border p-4 h-20 animate-pulse bg-champagne/5" />)}
        </div>
      ) : returns.length === 0 ? (
        <div className="py-16 text-center border border-dashed border-border">
          <RotateCcw className="h-10 w-10 mx-auto text-muted-foreground stroke-1 mb-4" />
            ...
            <div key={ret.id} className="border border-border">
              ...
              >
                <div>
                  <div className="flex items-center gap-3">
                    <span className={`border px-2 py-0.5 text-[10px] uppercase tracking-wider rounded ${RETURN_STATUS_COLORS[ret.status] || "bg-muted"}`}>
                ...
                {expandedId === ret.id ? <ChevronUp className="h-4 w-4 text-muted-foreground shrink-0" /> : <ChevronDown className="h-4 w-4 text-muted-foreground shrink-0" />}
              </div>
              ...
              {expandedId === ret.id && (
                <div className="border-t border-border bg-champagne/5 p-5 space-y-3 text-sm">
                  ...
                  {ret.comments && <p><span className="text-muted-foreground">Comments:</span> {ret.comments}</p>}
                  {ret.admin_notes && (
