...
import { HelpCircle, Plus, Send, ChevronDown, ChevronUp, Clock, CheckCircle2, AlertCircle, MessageSquare } from "lucide-react";
...
export const Route = createFileRoute("/dashboard/support")({
  head: () => ({
    meta: [
      { title: "Support — Drapeva" },
      { name: "description", content: "Create and view your support tickets." },
    ],
  }),
  component: Support,
});
...
const TICKET_STATUS_CONFIG = {
  ...
  in_progress: { label: "In Progress", color: "bg-amber-50 text-amber-700 border-amber-200", icon: AlertCircle },
...
function Support() {
  ...
  const replyMut = useMutation({
    mutationFn: ({ ticketId }: { ticketId: string }) => supportApi.addMessage(ticketId, {
      ticket_id: ticketId,
    ...
    <DashboardLayout title="Support" subtitle="Help & Support">
      ...
      {showForm && (
        ...
        >
          ...
          {orders.length > 0 && (
            <label className="block">
              <span className="eyebrow mb-1 block">Related Order <span className="normal-case text-muted-foreground">(optional)</span></span>
                ...
                >
                  <div>
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <span className={`border px-2 py-0.5 text-[10px] uppercase tracking-wider rounded inline-flex items-center gap-1 ${cfg.color}`}>
