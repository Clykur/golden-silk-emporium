...
export const Route = createFileRoute("/shop")({
  ...
  head: () => ({
    meta: [
      { title: "The Saree Catalog — Drapeva" },
      {
        name: "description",
        content: "Shop heirloom Indian sarees: Kanjivaram, Banarasi, Organza, Linen, and Designer weaves from the Drapeva atelier.",
      },
    ],
  }),
  component: Shop,
...
const FABRICS = ["Silk", "Kanjivaram", "Banarasi", "Organza", "Chiffon", "Linen", "Cotton", "Designer", "Handloom", "Contemporary"];
...
function Shop() {
  const { category, fabric, collection, occasion, search: searchParam } = Route.useSearch();
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedFabrics.length) {
      list = list.filter((p) => selectedFabrics.some((f) => (p.fabric || "").toLowerCase() === f.toLowerCase()));
    }
    if (selectedOccasions.length) {
      list = list.filter((p) => selectedOccasions.some((o) => (p.occasion || "").toLowerCase() === o.toLowerCase()));
    }
    if (selectedColors.length) {
      list = list.filter((p) => selectedColors.some((c) => (p.color || "").toLowerCase().includes(c.toLowerCase())));
    }
    if (inStockOnly) {
    ...
    return list;
  }, [allProducts, selectedFabrics, selectedOccasions, selectedColors, priceBandIndex, inStockOnly, sort]);
      ...
      <div className="border-b border-border bg-champagne/30">
        <div className="container-luxe py-8 md:py-8 text-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm text-muted-foreground">
            Every saree is carefully selected for its quality, comfort, and elegance, then delivered across India with reliable shipping and dedicated customer support.
          </p>
        </div>
      </div>
        ...
        <div className="grid gap-10 lg:grid-cols-[260px_1fr]">
          ...
          <aside className={`${open ? "fixed inset-0 z-50 overflow-y-auto bg-background p-6" : "hidden"} lg:static lg:block lg:p-0`}>
            ...
            <FilterGroup title="Fabric">
              {FABRICS.map((f) => (
                <Check key={f} label={f} checked={selectedFabrics.includes(f)} onChange={() => toggle(selectedFabrics, f, setSelectedFabrics)} />
              ))}
            </FilterGroup>

            <FilterGroup title="Price Range">
              {PRICE_BANDS.map((b, i) => (
                <Check key={b.label} label={b.label} checked={priceBandIndex === i} onChange={() => setPriceBandIndex(priceBandIndex === i ? null : i)} />
              ))}
            </FilterGroup>
            <FilterGroup title="Availability">
              <Check label="In Stock Only" checked={inStockOnly} onChange={() => setInStockOnly(!inStockOnly)} />
            </FilterGroup>

            <div className="mt-8 flex gap-3 lg:flex-col">
              <button onClick={() => setOpen(false)} className="flex-1 bg-foreground py-3 text-xs uppercase tracking-[0.2em] text-background lg:hidden font-medium">
                Show {filtered.length} pieces
              </button>
              <button onClick={clearAll} className="flex-1 border border-border py-3 text-xs uppercase tracking-[0.2em] hover:border-foreground font-medium transition-colors">
                Clear all
              </button>
            </div>
          </aside>
          ...
          <div>
            <div className="mb-8 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => setOpen(true)} className="inline-flex items-center gap-2 border border-border px-4 py-2 text-xs uppercase tracking-[0.2em] lg:hidden font-medium">
            ...
            {isLoading ? (
              ...
              <div className="grid place-items-center py-24 text-center">
                ...
                <button onClick={clearAll} className="mt-4 border-b border-foreground pb-1 eyebrow text-xs">
                  Clear filters
                </button>
              </div>
            ) : (
...
function Check({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) {
  return (
    <label className="flex cursor-pointer items-center gap-3 text-sm select-none">
      ...
      <span className={checked ? "text-foreground font-medium" : "text-muted-foreground hover:text-foreground"}>
        {label}
      </span>
    </label>
  );
